...
HuaweiPods 是面向小米 HyperOS 的 Xposed 模块。目前已对 **HUAWEI FreeBuds 3** 与 **HUAWEI FreeBuds Pro 3** 的已实现控制能力完成真机验证。
...
- 已在系统蓝牙中配对 HUAWEI FreeBuds 3 或 HUAWEI FreeBuds Pro 3。
...
首次启用时建议完整重启手机，确保蓝牙、系统设置、融合设备中心等进程都加载模块。之后连接已配对的受支持耳机。
...
4. 在系统蓝牙中断开再连接耳机；
5. 确认设备名称确实是 FreeBuds 3 或 FreeBuds Pro 3，而不是名称相似的其他型号。
